...
ins = ("INSERT INTO 'zones' ('id','name','expansion','mapid','scale','offset_x','offset_y','offset_z') "
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
for r in zone.values():
    try:
        data = (
            r['Id'], r['Name']['English'], r['Expansion'], r['MapId'], r['Scale'], r['Offset']['X'], r['Offset']['Y'],
            r['Offset']['Z'])
        cursor.execute(ins, data)
    except KeyError:
        pass
...
for r in hunt.values():
    data = (r['Id'], r['Name']['English'], r['Rank'], r['Expansion'], r['SpawnTimers']['Normal']['Minimum'],
            r['SpawnTimers']['Normal']['Maximum'])
    cursor.execute(ins, data)
